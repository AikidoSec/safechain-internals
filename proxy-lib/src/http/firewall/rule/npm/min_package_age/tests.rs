...
use crate::utils::time::SystemDuration;

use super::*;

use rama::http::{Body, BodyExtractExt as _};

fn get_package_age_cutoff_ts() -> SystemTimestampMilliseconds {
    SystemTimestampMilliseconds::now() - SystemDuration::days(2)
}
...
fn replaces_npm_install_accept_header() {
    ...
    let min_package_age = MinPackageAge::new(None);
...
async fn removes_versions_newer_than_48h() {
    ...
    let result = min_package_age
        .remove_new_packages(resp, get_package_age_cutoff_ts())
        .await
        .unwrap();
