use std::fmt;

use rama::{
    Service,
    error::{BoxError, ErrorContext as _, extra::OpaqueError},
    graceful::ShutdownGuard,
    http::{Request, Response, Uri},
    net::address::Domain,
    telemetry::tracing,
    utils::{
        str::arcstr::{ArcStr, arcstr},
        time::now_unix_ms,
    },
};

use crate::{
    endpoint_protection::{PackagePolicyDecision, PolicyEvaluator, RemoteEndpointConfig},
    http::firewall::{
        domain_matcher::DomainMatcher,
        events::{Artifact, BlockReason},
    },
    package::{
        malware_list::{LowerCaseEntryFormatter, RemoteMalwareList},
        released_packages_list::{LowerCaseReleasedPackageFormatter, RemoteReleasedPackagesList},
        version::PackageVersion,
    },
    storage::SyncCompactDataStorage,
};

#[cfg(feature = "pac")]
use crate::http::firewall::pac::PacScriptGenerator;

use super::{BlockedRequest, RequestAction, Rule};

mod parser;
use parser::{RubyPackage, is_gem_download_path, parse_package_from_path};

pub(in crate::http::firewall) struct RuleRuby {
    target_domains: DomainMatcher,
    remote_malware_list: RemoteMalwareList,
    remote_released_packages_list: RemoteReleasedPackagesList,
    remote_endpoint_config: Option<RemoteEndpointConfig>,
    policy_evaluator: Option<PolicyEvaluator>,
}

impl RuleRuby {
    pub(in crate::http::firewall) async fn try_new<C>(
        guard: ShutdownGuard,
        remote_malware_list_https_client: C,
        sync_storage: SyncCompactDataStorage,
        policy_evaluator: Option<PolicyEvaluator>,
        remote_endpoint_config: Option<RemoteEndpointConfig>,
    ) -> Result<Self, BoxError>
    where
        C: Service<Request, Output = Response, Error = OpaqueError> + Clone,
    {
        let remote_malware_list = RemoteMalwareList::try_new(
            guard.clone(),
            Uri::from_static("https://malware-list.aikido.dev/malware_ruby.json"),
            sync_storage.clone(),
            remote_malware_list_https_client.clone(),
            LowerCaseEntryFormatter,
        )
        .await
        .context("create remote malware list for ruby block rule")?;

        let remote_released_packages_list = RemoteReleasedPackagesList::try_new(
            guard,
            Uri::from_static("https://malware-list.aikido.dev/releases/ruby.json"),
            sync_storage,
            remote_malware_list_https_client,
            LowerCaseReleasedPackageFormatter,
        )
        .await
        .context("create remote released packages list for ruby block rule")?;

        Ok(Self {
            target_domains: ["rubygems.org"].into_iter().collect(),
            remote_malware_list,
            remote_released_packages_list,
            remote_endpoint_config,
            policy_evaluator,
        })
    }
}

impl fmt::Debug for RuleRuby {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("RuleRuby").finish()
    }
}

impl Rule for RuleRuby {
    #[inline(always)]
    fn match_domain(&self, domain: &Domain) -> bool {
        self.target_domains.is_match(domain)
    }

    #[cfg(feature = "pac")]
    #[inline(always)]
    fn collect_pac_domains(&self, generator: &mut PacScriptGenerator) {
        for domain in self.target_domains.iter() {
            generator.write_domain(&domain);
        }
    }

    async fn evaluate_request(&self, req: Request) -> Result<RequestAction, BoxError> {
        let path = req.uri().path();

        if !is_gem_download_path(path) {
            return Ok(RequestAction::Allow(req));
        }

        let Some(ruby_package) = parse_package_from_path(path) else {
            tracing::debug!(
                http.url.path = %path,
                "Ruby url looks like a .gem download but could not be parsed: passthrough"
            );
            return Ok(RequestAction::Allow(req));
        };

        tracing::debug!(
            http.url.path = %path,
            package.name = %ruby_package.fully_qualified_name,
            package.version = %ruby_package.version,
            "Ruby package download request"
        );

        if let Some(policy_evaluator) = self.policy_evaluator.as_ref() {
            let decision = policy_evaluator
                .evaluate_package_install("ruby", &ruby_package.fully_qualified_name);

            match decision {
                PackagePolicyDecision::Allow => {
                    return Ok(RequestAction::Allow(req));
                }
                PackagePolicyDecision::Defer => {}
                decision => {
                    return Ok(RequestAction::Block(BlockedRequest::blocked(
                        req,
                        Self::blocked_artifact(&ruby_package),
                        super::block_reason_for(decision),
                    )));
                }
            }
        }

        if self.is_package_listed_as_malware(&ruby_package) {
            return Ok(RequestAction::Block(BlockedRequest::blocked(
                req,
                Self::blocked_artifact(&ruby_package),
                BlockReason::Malware,
            )));
        }

        let cutoff_secs = self.get_package_age_cutoff_secs();
        if self.remote_released_packages_list.is_recently_released(
            &ruby_package.fully_qualified_name,
            Some(&PackageVersion::Semver(ruby_package.version.clone())),
            cutoff_secs,
        ) {
            tracing::info!(
                http.url.path = %path,
                package = %ruby_package,
                "blocked ruby gem download: package released too recently"
            );
            return Ok(RequestAction::Block(BlockedRequest::blocked(
                req,
                Self::blocked_artifact(&ruby_package),
                BlockReason::NewPackage,
            )));
        }

        tracing::debug!(
            http.url.path = %path,
            "Ruby gem does not contain malware: passthrough"
        );
        Ok(RequestAction::Allow(req))
    }
}

impl RuleRuby {
    const DEFAULT_MIN_PACKAGE_AGE_SECS: i64 = 48 * 3600;

    fn blocked_artifact(ruby_package: &RubyPackage) -> Artifact {
        Artifact {
            product: arcstr!("ruby"),
            identifier: ArcStr::from(ruby_package.fully_qualified_name.as_str()),
            display_name: None,
            version: Some(PackageVersion::Semver(ruby_package.version.clone())),
        }
    }

    fn get_package_age_cutoff_secs(&self) -> i64 {
        let maybe_ts = self.remote_endpoint_config.as_ref().and_then(|c| {
            c.get_ecosystem_config("ruby")
                .config()
                .and_then(|cfg| cfg.minimum_allowed_age_timestamp)
        });
        if let Some(ts_secs) = maybe_ts {
            return ts_secs;
        }
        (now_unix_ms() / 1000) - Self::DEFAULT_MIN_PACKAGE_AGE_SECS
    }

    fn is_package_listed_as_malware(&self, ruby_package: &RubyPackage) -> bool {
        self.remote_malware_list.has_entries_with_version(
            &ruby_package.fully_qualified_name,
            PackageVersion::Semver(ruby_package.version.clone()),
        )
    }
}

#[cfg(test)]
mod tests;
