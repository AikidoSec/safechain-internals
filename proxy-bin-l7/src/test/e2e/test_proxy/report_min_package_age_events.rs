...
use crate::{
    client::mock_server::malware_list::{
        ...
        FRESH_OPEN_VSX_EXTENSION_VERSION, FRESH_PYPI_PACKAGE_NAME, FRESH_PYPI_PACKAGE_VERSION,
...
#[tokio::test]
#[tracing_test::traced_test]
async fn test_report_open_vsx_min_package_age_events_posts_json_to_endpoint() {
    let capture_client = crate::client::new_http_client_for_internal(Executor::default()).unwrap();
    ...
    let client = runtime.client_with_http_proxy().await;
    let resp = client
        .post("https://open-vsx.org/vscode/gallery/extensionquery")
        .header("content-type", "application/json")
        .body("{}")
        .send()
        .await
    ...
    let version_strings: Vec<&str> = versions
        ...
        .collect();
    assert!(
        !version_strings.contains(&FRESH_OPEN_VSX_EXTENSION_VERSION),
        "fresh open-vsx version should have been suppressed from the response"
    );
    assert!(
        version_strings.contains(&"0.9.0"),
        "old stable version should remain in the response"
    ...
    tracing::info!(
        ...
        identifier = %first.artifact.identifier,
        suppressed_versions = ?first.suppressed_versions,
        "captured open-vsx min-package-age event"
    );

    let expected_id =
        format!("{FRESH_OPEN_VSX_EXTENSION_PUBLISHER}/{FRESH_OPEN_VSX_EXTENSION_NAME}");
    assert_eq!("open_vsx", first.artifact.product.as_str());
    assert_eq!(expected_id, first.artifact.identifier.as_str());
    assert_eq!(1, first.suppressed_versions.len());
    assert_eq!(
