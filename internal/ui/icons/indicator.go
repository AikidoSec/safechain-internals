...
func LayoutWarningIndicator(gtx layout.Context, size unit.Dp, bg, border, iconColor color.NRGBA) layout.Dimensions {
	dim := gtx.Dp(size)
	radius := dim / 4
	...
	// Draw alert triangle icon centered
	iconSize := unit.Dp(float32(size) * 0.5)
	offset := (dim - gtx.Dp(iconSize)) / 2

	defer op.Offset(image.Point{X: offset, Y: offset}).Push(gtx.Ops).Pop()
	LayoutAlertTriangle(gtx, iconSize, iconColor)
